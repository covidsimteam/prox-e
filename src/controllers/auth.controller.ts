...
export class AuthController extends Controller {
  ...
  private token(option: CovAuthOption, email?: string, pasword?: string): Observable<AuthResponse> {
    switch (+option) {
  ...
  public async signIn(@Body() creds: Authenticate): Promise<AuthResponse>{
    return this.token(CovAuthOption.signIn, creds.email, creds.password)?.toPromise();
  }
  ...
  public async signUp(@Body() creds: Authenticate): Promise<AuthResponse> {
    return this.token(CovAuthOption.signUp,creds.email, creds.password)?.toPromise();
  }
  ...
  public signOut(): Promise<AuthResponse> {
    return this.token(CovAuthOption.signOut)?.toPromise();
  }
