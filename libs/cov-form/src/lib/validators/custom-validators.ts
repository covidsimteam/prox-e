  ...
  static endDateValidator(): ValidatorFn {
    ...
    return (ac: AbstractControl): ValidationErrors => {
      ...
      if (!dateRange.endDate) {
        dateRange.endDate = ac;
        dateRange.startDate = ac.parent.get('startDate');
        ...
        dateRange.startDate.valueChanges.pipe(takeUntil(ngUnsubscribe)).subscribe(() => {
          dateRange.endDate.updateValueAndValidity();
        });
      }

      // if start date has not been given a value yet return back
      if (!dateRange.startDate || !dateRange.endDate.value) {
        return null;
      }

      // converts value from input type=date to Date obj for easy comparision
      if (
        new Date(dateRange.startDate.value).getTime() > new Date(dateRange.endDate.value).getTime()
      ) {
